...
Then type `start`. The program plays the fixed opening as text, accepts participant text, sends the finalized Compact v2 prompt and structured-output schema to the API, resolves the returned route locally, and prints the prerecorded-response text that would be played by Unity.
