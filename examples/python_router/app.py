...
ROUTER_PROMPT = (BASE_DIR / "prompts" / "router_prompt_compact_v2.md").read_text(encoding="utf-8")
def load_router_prompt(_variant: str = "compact_v2") -> str:
...
def call_router(request_obj,*,api_key,model,reasoning_effort,timeout,router_prompt=None):
    api_schema={k:v for k,v in RESULT_SCHEMA.items() if k not in {"$schema","title"}}
    payload={
        "model":model,
        "instructions":ROUTER_PROMPT if router_prompt is None else router_prompt,
...
def main():
    ...
    selected_prompt=ROUTER_PROMPT
    print(f"BC Router Reference v6.3 | model={args.model} | inputMode={args.input_mode} | prompt=compact_v2")
    ...
    while True:
        ...
        try:
            result,raw=call_router(req_obj,api_key=api_key,model=args.model,reasoning_effort=args.reasoning,timeout=args.timeout,router_prompt=selected_prompt)
